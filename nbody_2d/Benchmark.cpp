...
//  chunkResults.
//
//  func(scheduleType, chunkSize) debe llamar a zeroAccelerations()
//  y computeAccelerations(sched, chunk, ...) internamente.
// ─────────────────────────────────────────────────────────────
void Benchmark::runChunkAnalysis(int numThreads,
    ...
    // Mapeo de scheduleType → nombre legible (para log en consola)
    ...
    for (int sched : schedules) {
        for (int chunk : chunkSizes) {
            ...
            // Capturar sched y chunk por valor para el lambda
            int s = sched, c = chunk;
