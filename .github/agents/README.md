...
|---|---|---|---|---|
| **Documentador** (`documenter.py`) | Python 3 + reglas + Gemini 2.5 Flash (opcional) | Semanal (lunes 09:00 UTC) y al fusionar a `main` | Typo, enlace vacío, sección faltante con plantilla obvia → abre PR con el fix y etiqueta `agent:auto-fix`; si falla, abre issue | Explicar un kernel, decisiones de diseño → issue con `Requiere intervención humana: <motivo>` |
...
- **Sin duplicados:** no se abre un issue si ya existe uno abierto con el
  mismo título.
- **Auto-fix del documentador:** los hallazgos mecánicos abren un PR con
  la corrección aplicada usando la etiqueta `agent:auto-fix`. El PR nunca se
  fusiona automáticamente; la protección de `main` exige aprobación humana.
  Si la creación del PR falla, se abre un issue con la sugerencia como fallback.
